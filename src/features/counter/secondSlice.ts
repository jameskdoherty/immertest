...
import { original, isDraft, produce, current, enableMapSet } from "immer";
...
export const secondSlice = createSlice({
  ...
  reducers: {
    wrongHandler: (state, action: PayloadAction<any>) => {
      ...
      function arrayHandle(endNodeOnKeyPath: any) {
        ...
        switch (action.payload[2]) {
          case "update":
            ...
            const filtered = initialState.todosArray.filter((item: any) => item['id'] !== action.payload[1]['id']);
